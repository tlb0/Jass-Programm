	...
	internal class Program
	{
			...
			while (profiles[0] < 4)
			{
				while (profiles[1] < 4)
				{
					while (profiles[2] < 4)
					{
						while (profiles[3] < 4)
						{
							stats = new(profiles, agressiveness, strategies);
							Console.WriteLine(string.Join("", profiles));
							for (int i = 0; i < n; i++)
							{
								Console.WriteLine($"Game {i + 1}...");
								GameState game = new(false);
								data[i] = game.AiGame(false);
							}
							string filename = string.Join("", profiles);
		...
		static void RandomTest()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				for (int j = 0; j < n; j++)
				{
					Console.WriteLine($"Game {j + 1}...");
					GameState game = new(true);
					data[j] = game.RandomGame(0);
				}
			...
			public int[] AiGame(bool print)
			{
				...
				for (int i = 0; i < 9; i++)
				{
					for (int j = 0; j < 4; j++)
					{
						...
						List<int> playableCards = players[j].PlayableCards(this, false);
		...
		internal class Player
		{
			...
			// TODO: There may be an issue with this function
			...
			public List<int> PlayableCards(GameState gameState, bool alphaBeta)
			{
			...
			public void RealignBids(int card, Knowledge knowledge)
			{
				for (int i = 0; i < 4; i++)
				{
					if (i != ownerID)
					{
						...
						if (deck[card].isTrump == true)
						{
							// knowledge probabilities is either 0, 1, or undefined?
				...
				int depth, int bestMove, int optimizingPlayerID, int alpha, int beta)
		{
			...
			List<int> playableCards = player.PlayableCards(gameState, false);
		...
		static void Sort(List<int> cards, bool alphabeta)
		{
			if (alphabeta)
			{
				cards = cards.OrderBy(x => deck[x].isTrump).ThenByDescending(x => deck[x].pointValue).ToList();
				return;
			}

			cards = cards.OrderBy(x => deck[x].isTrump).ThenBy(x => deck[x].suit).ThenBy(x => deck[x].trickValue).ToList();
		}
...
{card.Name()}:  {p[0]}  {p[1]}  {p[2]}  {p[3]}");
		...
		static void WriteExcel(int[][] data, string filename)
		{
			Console.WriteLine("Opening document...");
			string filepath = "C:\\Users\\Admin\\source\\repos\\JassAlgorithmv9";
			string templateFilename = "\\JassDataTemplate.xlsx";
			Microsoft.Office.Interop.Excel.Application excel = new();
			Workbook workbook = excel.Workbooks.Open(filepath + templateFilename);
			Worksheet worksheet = workbook.Worksheets[1];

			for (int i = 0; i < data.Length; i++)
			{
				Console.WriteLine($"Writing data... ({i + 1}/{data.Length})");
